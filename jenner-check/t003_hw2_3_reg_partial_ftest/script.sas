/* HW2_3 — cooling method for gas turbines: partial F-test for the quadratic terms.
   The original script imported GASTURBINE-1.txt via PROC IMPORT from a SAS Studio
   home path; here the same GASTURBINE data is read from an inline DATALINES block
   (tab-delimited, identical values) so the analysis is self-contained. The
   downstream PROC REG models and the partial F-test are the author's own. */

data gasturbine;
  infile datalines dlm='09'x truncover;
  input ENGINE :$20. SHAFTS RPM CPRATIO INLET_TEMP EXH_TEMP AIRFLOW POWER HEATRATE;
  datalines;
